...
export function roomMirrorPlacement(entry,entries){
 ...
 if(entry.category==='food')return {x:307+(i%3)*15,y:87+Math.floor(i/3)*28,w:17,h:23,label:'팬트리'};
 if(entry.presentationRole==='sofa')return {x:66+(i%2)*25,y:342+Math.floor(i/2)*24,w:42,h:42,label:'소파'};
 if(entry.presentationRole==='lamp')return {x:332-i*18,y:400,w:44,h:50,label:'거실 스탠드'};
 if(entry.category==='living')return {x:134+(i%2)*20,y:386+Math.floor(i/2)*18,w:24,h:26,label:'거실 테이블'};
