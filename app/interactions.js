...
export class InteractionController {
 ...
 dispatch(e){
  ...
  if(e.type==='IMMEDIATE'){
   const permitted=(e.kind==='food'&&(['fridge','pantry'].includes(this.objectId)&&this.phase==='engaged'||this.owner==='modal:pantry')&&['milk','water','vitamin'].includes(e.productId)&&e.quantity>0)||(e.kind==='lamp'&&(this.objectId==='sofa'&&this.phase==='engaged'||this.owner==='modal:lamp'));
 ...
 view(){const id=this.objectId,p=this.duration?clamp(this.elapsed/this.duration):0,seat=id==='sofa'||id==='vanity'?this.value(id):0;let pose='idle';if(this.step==='switch'||this.step==='open'||this.step==='toggle'||this.phase==='exiting'&&id!=='sofa'&&id!=='vanity')pose='reach';if(this.step==='browse')pose='browse';if(this.step==='sit')pose='sit-down';if(seat&&this.phase==='engaged')pose='seated-idle';if(this.phase==='exiting'&&(id==='sofa'||id==='vanity'))pose='stand-up';if(this.step==='gesture')pose=id==='wardrobe'?'change-clothes':'use-cosmetic';const bed=id==='bed'?this.value('bed'):0;if(id==='bed')pose=this.phase==='exiting'?'get-up':this.step==='lie'?'lie-down':this.phase==='engaged'?'lying-idle':'idle';const offset=OBJECTS[id]?.offset||{x:0,y:0},occupancy=id==='bed'?bed:seat;return {objectId:id,primary:this.primary,phase:this.phase,step:this.step,pose,progress:p,seatProgress:seat,bedProgress:bed,renderOffset:{x:offset.x*occupancy,y:offset.y*occupancy},wardrobeOpen:this.value('wardrobe'),fridgeOpen:this.value('fridge'),windowOpen:this.value('window'),browseProgress:this.step==='browse'&&!this.reduced?p:null,selectedFood:this.selectedFood,sofaOccupied:this.objects.sofa.stable==='occupied',vanityOccupied:this.objects.vanity.stable==='occupied',heldProductId:this.step==='gesture'&&id==='vanity'&&p>=.25&&p<.75?this.action?.productId:null};}
}
