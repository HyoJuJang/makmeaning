...
document.addEventListener('click',event=>{if(event.target?.closest?.('a[data-food-entry]'))saveHomeReturn();},true);
...
document.querySelector('#reset').addEventListener('click',()=>{if(active)closeRoom();handleEffects(controller.dispatch({type:'RESET'}));state=initial();sceneSituation=null;sceneBudget=null;sceneStep='choose';position={...START};direction='down';returnScrollY=null;try{localStorage.removeItem(FOOD_KEY);}catch{}try{sessionStorage.removeItem(HOME_RETURN_KEY);}catch{}persist('데모를 초기화했어요');});
