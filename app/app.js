...
function productAction(id){
 ...
 else if(category==='food'&&product.category==='food'&&['pantry','fridge'].includes(controller.objectId)&&controller.phase==='engaged'&&state.foodQuantity[id]>0){handleEffects(controller.dispatch({type:'SELECT_FOOD',productId:id}));handleEffects(controller.dispatch({type:'EAT',productId:id,quantity:state.foodQuantity[id],consumptionMode:foodConsumptionMode(product)}));}
