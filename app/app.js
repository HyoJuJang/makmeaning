...
import {demoStateKey,personalStateKey,readDemoState,resetDemoState,outfitArtKey,consumeOwnedFood,updateDemoState} from './demo-state.js';
import {activePersonaId,selectDemoPersona} from './demo-persona.js';
...
import {placedMirrorProducts,roomMirrorPlacement,vanityMirrorClip} from './room-mirror.js';
...
const ROOM_RETURN_BASE='gscene-room-return-v1';
...
function validateHome(data){
 ...
 const personas=Array.isArray(data.personas)?data.personas.filter(p=>p.id===`demo-${p.avatarId}`&&AVATARS.some(a=>a.id===p.avatarId)&&typeof p.name==='string'&&typeof p.theme==='string'):undefined;
 return {demo:data.demo,personas,user:{...data.user,name:data.user.name.trim().slice(0,40),avatarId:avatarById(canonicalAvatarId(data.user.avatarId)).id},categories,purchases:CATEGORIES.flatMap(category=>categories[category].ownedProducts)};
}
...
const KEY=demoStateKey(home),ROOM_RETURN_KEY=personalStateKey(ROOM_RETURN_BASE,home);
...
function preloadEatingAsset(){if(preloadedEatingAvatar===state.avatarId||typeof Image!=='function')return;preloadedEatingAvatar=state.avatarId;const image=new Image();image.src=`/assets/avatars/${state.avatarId}-meal-back.png`;}
...
function productHTML(p,category=active){let label='',disabled=false,meta=`${escapeHTML(p.purchasedAt)} 가상 구매 · ${productLocation(p)}`;
 ...
 if(category==='food'){meta+=` · 데모 잔량 ${state.foodQuantity[p.id]}회`;label=state.foodQuantity[p.id]?foodActionLabel(p):'다 먹었어요';disabled=!state.foodQuantity[p.id];}
 ...
 const artwork=gameItemArt(p)||(p.catalogSource==='shared-products'||p.imageKind==='product-photo'?'<span data-artwork-state="unavailable" style="display:grid;place-items:center;width:60px;height:60px;background:#edf0e4;color:#68775e;font-size:10px;text-align:center">공간 이미지<br>준비 중</span>':`<img src="${p.imageUrl}" alt="" width="60" height="60" loading="lazy" data-illustration="${p.illustrationKey}">`);
 return `<div class="product" data-product="${p.id}" data-illustration="${p.illustrationKey}" data-room-slot="${p.roomSlot}"><div class="thumb" aria-hidden="true">${artwork}</div><div class="product-info">${category==='food'&&['fridge','pantry'].includes(controller.objectId)?`<button class="food-select" data-food-select="${p.id}" aria-pressed="${controller.selectedFood===p.id}">${escapeHTML(p.name)}</button>`:`<h3 class="product-name">${escapeHTML(p.name)}</h3>`}<p class="product-meta">${meta}</p><p class="product-meta product-price">카탈로그 참고가 ${p.price.toLocaleString('ko-KR')}원</p>${p.illustrationKey==='cushion'?'<p class="product-meta">소파에 놓여 있어요</p>':''}</div>${label?`<button class="action" data-action="${p.id}" aria-label="${escapeHTML(p.name)} ${label}" ${disabled?'disabled':''}>${label}</button>`:''}</div>`;
}
...
function closeRoom(){controller.dispatch({type:'CLOSE_MODAL'});active=null;draftAvatar=null;document.querySelector('#modal-root').innerHTML='';document.body.style.overflow='';document.querySelector('#app').inert=false;returnFocus?.focus();inform('바닥을 눌러 걸어보세요');}
function renderAvatarChoices(){document.querySelector('.avatar-choices').innerHTML=AVATARS.map(a=>`<button class="avatar-choice" data-avatar="${a.id}" aria-pressed="${draftAvatar===a.id}"><span class="avatar-preview"><svg viewBox="0 0 40 64" aria-hidden="true">${avatarSVG(a.id,'base')}</svg><span class="selection-tick" aria-hidden="true">✓</span></span><strong>${escapeHTML(home.personas?.find(p=>p.avatarId===a.id)?.name||a.name)}</strong><small>${escapeHTML(home.personas?.find(p=>p.avatarId===a.id)?.theme||a.note)}</small></button>`).join('');}
function showAvatarPicker(){stopMovement();paintActor();active='avatar';draftAvatar=state.avatarId;returnFocus=document.querySelector('#avatar-picker');document.querySelector('#modal-root').innerHTML=`<div class="overlay"><section class="sheet avatar-sheet" role="dialog" aria-modal="true" aria-labelledby="sheet-title" aria-describedby="sheet-desc"><div class="handle"></div><div class="sheet-head"><div><span class="sheet-eyebrow">MY CHARACTER</span><h2 id="sheet-title">내 공간 속의 나</h2></div><button class="close" aria-label="닫기">×</button></div><p class="sheet-desc" id="sheet-desc">${home.personas?'캐릭터를 바꾸면 구매한 상품과 추천도 함께 바뀌어요.':'마음에 드는 모습으로 골라보세요.'}</p><div class="sheet-content"><div class="avatar-choices"></div><p class="avatar-outfit-note">${home.personas?'선택은 데모 사용자 전환이며 실제 로그인은 아니에요.':state.outfitId==='base'?'캐릭터마다 어울리는 기본 옷으로 시작해요.':'미리보기는 기본 옷, 지금 고른 착장은 그대로 유지돼요.'}</p></div><button class="apply-avatar">이 캐릭터로 적용하기</button></section></div>`;renderAvatarChoices();document.body.style.overflow='hidden';document.querySelector('#app').inert=true;document.querySelector(`[data-avatar="${draftAvatar}"]`).focus();}
...
function handleEffects(effects){for(const effect of effects){
 ...
 if(effect.type==='action-start'){renderTray(true);if(controller.step==='eat')inform(trayStatus());ensureInteractionVisible();}
 if(effect.type==='tray')renderTray(true);
 if(effect.type==='commit'){commitProduct(effect);renderTray(true);}
 if(effect.type==='exit'){renderTray(true);if(!effect.intent){scene.focus({preventScroll:true});inform('바닥을 눌러 걸어보세요');}}
...
function commitProduct({kind,productId:id}){
 ...
 if(kind==='food'&&product?.category==='food'){patch=current=>consumeOwnedFood(home,current,id);message=`${product.name} ${foodConsumptionMode(product)==='drink'?'잘 마셨어요':'잘 먹었어요'}`;}
...
let foodActionGesture=null;
function productAction(id,trigger){
 ...
 else if(category==='food'&&product.category==='food'&&['pantry','fridge'].includes(controller.objectId)&&controller.phase==='engaged'&&state.foodQuantity[id]>0){
  handleEffects(controller.dispatch({type:'SELECT_FOOD',productId:id}));
  const effects=controller.dispatch({type:'EAT',productId:id,quantity:state.foodQuantity[id],consumptionMode:foodConsumptionMode(product)});
  if(effects.some(effect=>effect.type==='action-start'))foodActionGesture=trigger?.detail>0?{x:trigger.clientX,y:trigger.clientY,time:trigger.timeStamp,actionId:controller.action.id}:null;
...
// Tray reflow can put the floor or a different control beneath the same gesture.
document.addEventListener('click',e=>{const gesture=foodActionGesture,age=e.timeStamp-gesture?.time;
 if(!gesture||controller.step!=='eat'||controller.action?.id!==gesture.actionId||!(e.detail>0)||age<0||age>350||Math.hypot(e.clientX-gesture.x,e.clientY-gesture.y)>24)return;
...
window.addEventListener('pageshow',event=>{if(event.persisted){if(home.personas&&activePersonaId()!==home.user.id){window.location.reload();return;}categoryNavigationInFlight=false;state=readDemoState(home,localStorage);try{sessionStorage.removeItem(ROOM_RETURN_KEY);}catch{}handleEffects(controller.dispatch({type:'BLUR'}));renderHouse();}});
...
function tick(time){const dt=Math.min((time-lastTime)/1000||0,.04);lastTime=time;const previous={...position};
 if(!active&&!document.hidden&&controller.owner==='movement'){
  if(keys.size||pressPulses.size){const inputs=new Set([...keys,...pressPulses]);let dx=0,dy=0;const inputDt=keys.size?dt:Math.max(dt,1/60);for(const key of inputs){dx+=keyVector[key][0];dy+=keyVector[key][1];}pressPulses.clear();const length=Math.hypot(dx,dy);if(length)position=moveWithCollision(position,dx/length*104*inputDt,dy/length*104*inputDt);}
   ...
   if(!path.length){document.querySelector('.destination-mark').hidden=true;const arrived=pendingInteraction;pendingInteraction=null;if(arrived){objectTrigger=arrived.trigger;handleEffects(controller.dispatch({type:'ARRIVED',id:arrived.id,epoch:arrived.epoch,pathEmpty:true,position}));}else{controller.dispatch({type:'MOVE_STOP'});inform('바닥을 눌러 걸어보세요');}}
