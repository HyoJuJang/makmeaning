...
function navigateToCategory(category){const route=CATEGORY_ROUTES[category];if(!route||categoryNavigationInFlight)return;categoryNavigationInFlight=true;stopMovement();try{localStorage.setItem(KEY,JSON.stringify(state));sessionStorage.setItem(ROOM_RETURN_KEY,JSON.stringify({position,direction,window:controller.objects.window.stable,savedAt:Date.now()}));}catch{}window.location.assign(route.href);}
...
function renderTray(force=false){const id=controller.objectId,root=document.querySelector('#tray-root');const stamp=JSON.stringify([id,controller.phase,controller.step,controller.trayExpanded,controller.selectedFood,state,controller.objects.window.stable]);if(!force&&stamp===lastTray)return;lastTray=stamp;if(!id){root.innerHTML='';return;}const object=OBJECTS[id],busy=controller.phase!=='engaged',expanded=controller.trayExpanded&&!busy;const focused=document.activeElement?.dataset;const focusAction=focused?.action,focusFood=focused?.foodSelect,focusTray=focused?.tray;
 root.innerHTML=`<section data-object="${id}" data-phase="${controller.phase}" class="context-tray ${expanded?'expanded':''}" aria-label="${object.name} 사용"><div class="tray-head"><h2 id="tray-title" tabindex="-1">${trayStatus()}</h2>${id==='window'?`<button data-tray="window" ${busy?'disabled':''}>${controller.objects.window.stable==='open'?'창문 닫기':'창문 열기'}</button>`:`<button data-tray="expand" aria-expanded="${expanded}" aria-label="${expanded?'상품 목록 접기':object.button}" ${busy?'disabled':''}>${expanded?'접기':object.button}</button>`}${OBJECT_CATEGORIES[id]&&!busy?`<button class="tray-scene-link" data-tray="category">${CATEGORY_ROUTES[OBJECT_CATEGORIES[id]].label} 둘러보기 ↗</button>`:''}<button class="tray-exit" data-tray="exit" aria-label="${object.exit}" title="${object.exit}">×</button></div>${expanded?`<div class="tray-products">${products.filter(p=>p.category===object.category).map(p=>productHTML(p,object.category)).join('')}</div>`:''}</section>`;
...
document.querySelector('#reset').addEventListener('click',()=>{if(active)closeRoom();try{localStorage.removeItem('gscene-scene-fashion-v1');localStorage.removeItem('gscene-scene-living-v1');}catch{}handleEffects(controller.dispatch({type:'RESET'}));state=initial();sceneSituation=null;sceneBudget=null;sceneStep='choose';position={...START};direction='down';persist('데모를 초기화했어요');});
document.querySelector('#tray-root').addEventListener('click',e=>{e.stopPropagation();const command=e.target.closest('[data-tray]')?.dataset.tray;if(command==='category'){requestIntent({type:'object',id:controller.objectId,trigger:objectTrigger});return;}if(command==='exit'){handleEffects(controller.dispatch({type:'CANCEL'}));return;}if(command==='expand'){handleEffects(controller.dispatch({type:'EXPAND'}));return;}if(command==='window'){handleEffects(controller.dispatch({type:'WINDOW_TOGGLE'}));ensureInteractionVisible();return;}const food=e.target.closest('[data-food-select]');if(food){handleEffects(controller.dispatch({type:'SELECT_FOOD',productId:food.dataset.foodSelect}));return;}const button=e.target.closest('[data-action]');if(button&&!button.disabled)productAction(button.dataset.action);});
...
window.addEventListener('pageshow',event=>{if(event.persisted){categoryNavigationInFlight=false;handleEffects(controller.dispatch({type:'BLUR'}));}});
