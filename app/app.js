...
const HOME_RETURN_KEY='gscene-home-return-v1';
const FOOD_KEY='gscene-food-v1';
const CATALOG_KEYS=['gscene-catalog-food-v1','gscene-catalog-beauty-v1'];
...
try{
 const saved=JSON.parse(sessionStorage.getItem(HOME_RETURN_KEY));
 sessionStorage.removeItem(HOME_RETURN_KEY);
 if(saved&&isWalkable(saved.position?.x,saved.position?.y)&&['up','down','left','right'].includes(saved.direction)){
  position={x:saved.position.x,y:saved.position.y};direction=saved.direction;
  if(Number.isFinite(saved.scrollY)&&saved.scrollY>=0&&saved.scrollY<=100000)returnScrollY=saved.scrollY;
 }
}catch{}
function saveHomeReturn(){
 try{sessionStorage.setItem(HOME_RETURN_KEY,JSON.stringify({position,direction,scrollY:Number.isFinite(window.scrollY)?window.scrollY:0}));}catch{}
}
// Full document navigation starts each page with its own event listeners.
// Capture also handles the fridge tray, whose bubbling click handler stops propagation.
document.addEventListener('click',event=>{if(event.target?.closest?.('a[data-food-entry],a[data-catalog-entry="food"],a[data-catalog-entry="beauty"]'))saveHomeReturn();},true);
window.addEventListener('pagehide',saveHomeReturn);
...
document.querySelector('#reset').addEventListener('click',()=>{if(active)closeRoom();handleEffects(controller.dispatch({type:'RESET'}));state=initial();sceneSituation=null;sceneBudget=null;sceneStep='choose';position={...START};direction='down';returnScrollY=null;for(const key of [FOOD_KEY,...CATALOG_KEYS]){try{localStorage.removeItem(key);}catch{}}try{sessionStorage.removeItem(HOME_RETURN_KEY);}catch{}persist('데모를 초기화했어요');});
...
scene.addEventListener('click',e=>{if(e.target.closest('button,a[href]')||active)return;scene.focus({preventScroll:true});const bounds=scene.getBoundingClientRect(),target={x:(e.clientX-bounds.left)/bounds.width*400,y:(e.clientY-bounds.top)/bounds.height*600};requestIntent({type:'move',target});});
...
function tick(time){const dt=Math.min((time-lastTime)/1000||0,.04);lastTime=time;const previous={...position};
 ...
 paintActor();paintObjects();renderTray();if(returnScrollY!==null){if(typeof window.scrollTo==='function')window.scrollTo({top:returnScrollY,behavior:'instant'});returnScrollY=null;}requestAnimationFrame(tick);
}
