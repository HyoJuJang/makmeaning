...
function renderSceneSheet(){document.querySelector('#modal-root').innerHTML=sceneSheetHTML(sceneSituation,sceneBudget,sceneStep);document.querySelector('.scene-content').scrollTop=0;}
function showScene(){stopMovement();paintActor();active='scene';sceneStep='choose';returnFocus=document.querySelector('#scene-entry');renderSceneSheet();document.body.style.overflow='hidden';document.querySelector('#app').inert=true;document.querySelector('.close').focus({preventScroll:true});}
...
function handleSceneClick(target){
 ...
 if(action==='start'&&validScene(sceneSituation,sceneBudget)){sceneStep='summary';renderSceneSheet();document.querySelector('#sheet-title').focus({preventScroll:true});}
 if(action==='edit'){sceneStep='choose';renderSceneSheet();document.querySelector(`[data-scene-situation="${sceneSituation}"]`).focus({preventScroll:true});}
