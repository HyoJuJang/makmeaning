...
import type { AssetLibraryResult, AssetStatus, Domain, ProductWithAsset } from '../../src/lib/game-asset-types';
...
export function AssetLibrary({ initial }: { initial: AssetLibraryResult }) {
  ...
  return <main className={styles.library}>
    ...
    <section className={styles.intro}>
      ...
      <div className={styles.stats}>
        ...
        <div><strong>{summary.assetStatuses.ready}</strong><span>이미지 연결 완료</span></div>
      </div>
    </section>
    <section className={styles.catalog} aria-label="상품 에셋 목록">
      ...
      <div className={styles.filters}>
        ...
        <span role="status">{loading ? '불러오는 중…' : `${result.total.toLocaleString()}개 상품`}</span>
      </div>
      ...
      {!result.products.length && <p className={styles.noResults}>조건에 맞는 상품이 없어요. 다른 상품 번호나 공간으로 찾아보세요.</p>}
      {result.pages > 1 && <nav className={styles.pagination} aria-label="목록 페이지"><button disabled={loading || result.page <= 1} onClick={() => setPage(result.page - 1)}>← 이전</button><span>{result.page} / {result.pages}</span><button disabled={loading || result.page >= result.pages} onClick={() => setPage(result.page + 1)}>다음 →</button></nav>}
    </section>
