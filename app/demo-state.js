...
export function selectWardrobeProducts(home,state,limit=4){
 const confirmed=normalizeDemoState(home,state).outfitId;
 const count=Number.isInteger(limit)?Math.max(0,Math.min(4,limit)):4;
 return home.purchases.filter(p=>p.category==='fashion').sort((a,b)=>Number(b.id===confirmed)-Number(a.id===confirmed)||b.purchasedAt.localeCompare(a.purchasedAt)||a.id.localeCompare(b.id)).slice(0,count);
}
