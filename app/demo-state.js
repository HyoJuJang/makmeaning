...
import {personaStorageKey} from './persona-browser.js';
// One confirmed state per fictional buyer. Rendering keys never identify products.
...
export const demoStateKey=home=>personaStorageKey(DEMO_STATE_KEY,home);
...
export function updateDemoState(home,current,patchOrUpdater,storage){
 ...
 try{
  ...
  const stale=failedStoreSnapshots.get(store)?.has(demoStateKey(home))&&failedStoreSnapshots.get(store).get(demoStateKey(home))===snapshot;
...
export function resetDemoState(home,storage){const state=initialDemoState(home);try{const store=storage||globalThis.localStorage;for(const key of DEMO_PERSONAL_KEYS)store.removeItem(personaStorageKey(key,home));saveDemoState(home,state,store);}catch{}return state;}
