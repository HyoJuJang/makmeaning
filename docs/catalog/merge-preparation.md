...
> 통합 후 기준: `docs/design/food_beauty_merge_review.md`. 아래는 동료 브랜치의 병합 전 기록이다. 실제 통합에서는 최신 main의 첫 탭 interaction → 준비 후 재탭 이동, 기존 복귀 저장 키, 네 카테고리 SVG 메뉴 및 초기 착장 보완을 우선한다. 즉시 이동 링크와 별도 홈 복귀 구현은 적용하지 않는다.
