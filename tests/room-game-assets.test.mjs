...
const rows=demoHome.purchases.map(p=>({prd_id:p.id,domain:p.category,view_name:p.name,discprice:p.price,cate1_nm:null,cate2_nm:null,cate3_nm:null,cate4_nm:null,brand_name:null}));
...
test('room resolves owned IDs once and preserves product source and unknown artwork',async()=>{
 let calls=0;
 const home=await resolveDemoHome(repo,async ids=>{calls++;assert.deepEqual(ids,rows.map(p=>p.prd_id));return [mapping]});
 ...
 const failed=await demoHomeResponse(()=>repo,async()=>{throw Error('postgres://private:secret@host')});
