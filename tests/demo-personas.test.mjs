...
test('the approved four purchase sets resolve exact IDs from the product source and reviewed assets',async()=>{
 ...
 for(const choice of demoPersonaChoices){
  const source=getPersonaSource(choice.id),home=await resolveDemoHome(repository,source.user.id,lookup);
  ...
  for(const category of ['food','beauty']){
   const response=await demoCatalogResponse(category,()=>repository,new Request('https://demo.example',{headers:{cookie:'gscene-persona='+source.user.id}}),lookup);
...
test('missing or wrong-category products fail honestly; an absent image stays unlinked',async()=>{
 ...
 const failed=await demoHomeResponse(()=>({find:async()=>null}),undefined,lookup);
 assert.equal(failed.status,503);
 const home=await resolveDemoHome(repository,source.user.id,async()=>[]);
