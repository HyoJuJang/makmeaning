...
function homeRows(category: CatalogCategory): CatalogProduct[] {
  return demoHome.purchases.filter(purchase => purchase.category === category).map(purchase => ({
    prd_id: purchase.id, view_name: purchase.name, price: purchase.price,
    cate1_nm: category === 'food' ? '식품' : '뷰티', cate2_nm: homeCategories[purchase.id][0],
    cate3_nm: homeCategories[purchase.id][1], cate4_m: '', brd_mn: '', domain: category === 'food' ? '푸드' : '뷰티',
  }));
}

/** Fictional catalog rows with only the nine promised source columns. */
...
function catalog(category: CatalogCategory): DemoCatalog {
  return {
    category,
    user: { id: demoHome.user.id, name: demoHome.user.name, avatarId: demoHome.user.avatarId },
    initialOutfitId: demoHome.purchases.find(purchase => purchase.category === 'fashion' && purchase.state.wearing === true)?.id,
    products: adaptCatalogProducts(demoCatalogRows[category], presentation),
    purchases: demoHome.purchases.filter(purchase => purchase.category === category)
      .map(purchase => ({ productId: purchase.id, purchasedAt: purchase.purchasedAt })),
    initialCart: [],
  };
}
